    ...
    const renderCard = categories.map((card) => {
        return (
            ...
            <div key={card.id} className=' flex-1 basis-[300px] '>
                {/* card Image */}
                <div  className='flex justify-center w-full min-h-[15vh] md:min-h-[20vh] relative' ><img className= 'h-30 md:h-45 absolute -bottom-10 ' src={card.img} alt="" /></div>

                {/* card Content  */}
                <div className='bg-zinc-200 pt-10 md:pt-20 pb-5 px-5 rounded-2xl'>
                    <h3 className='text-2xl md:text-3xl font-bold  text-zinc-800'>{card.title}</h3> 
                    <p className=' text-zinc-700 mt-5 mb-5 md:mb-10 text-sm md:text-lg'>{card.description}</p>
                    <Link to={card.path}
